-- payer_depth_x402.sql — AGE-138 repeat-depth ranking (AGE-133 phase 0).
--
-- Per x402 seller on Base, 30d: how many payers came BACK (a second leg to the
-- SAME recipient), and how many payers look like verification bots (one leg
-- here, one leg per recipient everywhere). Payer counts are manufacturable —
-- fifty bots paying once each are fifty "unique payers" — repeat depth is not.
--
-- Same denominator as census_sellers_x402.sql (v2): EIP-3009 AuthorizationUsed
-- + tx sender ∈ x402scan's facilitator registry (128 Base relayers). The
-- recipient filter is WIDER than the census's metered-seller cut on purpose —
-- ≥2 distinct payers and mean leg ≤ $5 — so small sellers (us: 9 payers) and
-- the $1–5 inference tier are in the map too. ~1–2k rows.
--
-- Per (recipient, payer) weight — the "payer_weight" of AGE-138:
--   legs_here >= 2                → 1.0   (came back: a customer)
--   legs_here == 1                → 0.2 + 0.8 * (1 - fanout)
--     fanout = distinct recipients ÷ legs, over ALL this payer's x402 legs in
--     the window. A prober pays 65 recipients once each: fanout 1.0 → 0.2.
--     An established buyer (140 legs to A, 1 to B): fanout ≈ 0.014 → B gets
--     ~0.99 for that payer — trying a new provider is not bot behaviour.
--   A payer with exactly one leg in the whole window has fanout 1.0 → 0.2.
--
-- Output columns (one row per recipient):
--   payers, legs, usd, mean_leg                 — raw 30d
--   returning_payers, retention                 — payers with >= 2 legs here ÷ payers
--   effective_payers, payer_quality             — Σ weight, and Σ weight ÷ payers
--   prober_share                                — payers with weight < 0.5 ÷ payers
--   p50_legs_per_payer, top_payer_share         — fleet shape: repeat depth rewards a
--                                                 fleet that repeats; these expose it
--   first_leg, last_leg
-- Window hardcoded (30 days). Run the script with --no-params / --use-latest.
...
-- Every x402 payer's market-wide shape (computed BEFORE the seller cut, so a
-- prober's legs to non-metered recipients still count toward its fanout).
payer_shape AS (
    SELECT payer,
           COUNT(*)                  AS legs_total,
           COUNT(DISTINCT recipient) AS recipients_total,
           CAST(COUNT(DISTINCT recipient) AS double) / COUNT(*) AS fanout
    FROM legs_all
    GROUP BY payer
),
-- Sellers in the map: paid per call by more than one wallet.
sellers AS (
    SELECT recipient
    FROM legs_all
    GROUP BY recipient
    HAVING COUNT(DISTINCT payer) >= 2 AND AVG(usd) <= 5.0
),
pairs AS (
    SELECT l.recipient, l.payer,
           COUNT(*)        AS legs_here,
           SUM(l.usd)      AS usd_here,
           MIN(block_time) AS first_leg,
           MAX(block_time) AS last_leg
    FROM legs_all l
    JOIN sellers s ON s.recipient = l.recipient
    GROUP BY l.recipient, l.payer
),
weighted AS (
    SELECT p.recipient, p.payer, p.legs_here, p.usd_here, p.first_leg, p.last_leg,
           ps.fanout,
           CASE WHEN p.legs_here >= 2 THEN CAST(1.0 AS double)
                ELSE CAST(0.2 + 0.8 * (1.0 - ps.fanout) AS double) END AS weight
    FROM pairs p
    JOIN payer_shape ps ON ps.payer = p.payer
)
SELECT recipient,
       COUNT(*)                                            AS payers,
       SUM(legs_here)                                      AS legs,
       SUM(usd_here)                                       AS usd,
       SUM(usd_here) / SUM(legs_here)                      AS mean_leg,
       SUM(CASE WHEN legs_here >= 2 THEN 1 ELSE 0 END)     AS returning_payers,
       CAST(SUM(CASE WHEN legs_here >= 2 THEN 1 ELSE 0 END) AS double) / COUNT(*) AS retention,
       SUM(weight)                                         AS effective_payers,
       SUM(weight) / COUNT(*)                              AS payer_quality,
       CAST(SUM(CASE WHEN weight < 0.5 THEN 1 ELSE 0 END) AS double) / COUNT(*) AS prober_share,
       -- fleet shape (what repeat depth does NOT catch — a fleet that repeats):
       approx_percentile(legs_here, 0.5)                   AS p50_legs_per_payer,
       MAX(usd_here) / SUM(usd_here)                       AS top_payer_share,
       MIN(first_leg)                                      AS first_leg,
       MAX(last_leg)                                       AS last_leg
FROM weighted
GROUP BY recipient
ORDER BY usd DESC
